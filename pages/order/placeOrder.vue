	...
	<view>
		...
		<view class="address-section" @tap="selectAddress">
			<view class="order-content">
				<text class="mallicon icon-shouhuodizhi"></text>
				<view class="cen">
					<view class="top">
						<text class="name" >{{addressData.RecieveName}}</text>
						<text class="mobile">{{addressData.MobilePhone}}</text>
					</view>
					<text class="address">{{addressData.Address}} {{addressData.AddressDetail}}</text>
				</view>
		...
		<view class="goods-section">
			<!-- 商品列表 -->
			<view class="g-item" v-for="(Goods,index) in transGoodsList" :key="index">
				<view class="image-wrapper">
					<image :src="Goods.DefaultPicURL" :class="[Goods.loaded]" mode="aspectFill" lazy-load @load="onImageLoad('transGoodsList', index)"
					 @error="onImageError('transGoodsList', index)"></image>
				</view>
				<view class="right">
					<text class="title clamp">{{Goods.GoodsName}}</text>
					<text class="spec">{{Goods.GoodsModel}}</text>
					<view class="price-box">
						<text class="price">￥{{Goods.Price}}</text>
						<text class="number">x {{Goods.CartNum}}</text>
					</view>
				</view>
			</view>
		</view>
		...
		<view class="footer">
			<view class="price-content">
				...
				<text class="price">{{(totalMoney-couponMoney).toFixed(2)}}</text>
			</view>
		...
		<view class="mask" @touchmove.stop.prevent="stopPrevent" :class="maskState===0 ? 'none' : maskState===1 ? 'show' : ''"
		 @click="toggleMask">
			<scroll-view scroll-y="true" class="mask-content" @click.stop.prevent="stopPrevent">
				<!-- 优惠券页面，仿mt -->
				<view class="coupon-item" v-for="(item,index) in couponList" :key="index">
					<view class="con">
						<view class="left">
							<text class="title">{{item.title}}</text>
							<text class="time">有效期至{{item.endDate}}</text>
						</view>
						<view class="right">
	...
	export default {
		...
		onLoad(option) {
			...
			this.transGoodsList.forEach(Goods => {
				Goods.loaded = '';
				total += Number(Goods.Price) * Number(Goods.CartNum);
			})
