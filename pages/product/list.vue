		...
		<view class="content-body">
			<view class="navbar" :style="{ position: headerPosition, top: headerTop }">
				...
				<view class="nav-item" :class="{ current: filterIndex === 3 }" @click="toggleCateMask('Cate', 'show')">
					<text>筛选</text>
					<view class="p-box"><text class="mallicon icon-shang"></text></view>
				</view>
			</view>
			...
			<view
				...
				@click="toggleCateMask('Cate', 'none')"
			>
				<view class="cate-content" @click.stop.prevent="stopPrevent" @touchmove.stop.prevent="stopPrevent">
					...
					<view style="display: flex;flex-direction: row;justify-content: space-between;height: 110rpx;padding: 20rpx 40rpx;font-size: 30rpx;">
						<text style="width: 45%;display: flex;flex-direction: row;justify-content: center;border: 1px solid #b4b4b4;align-items: center;border-radius: 50rpx;">
							重置
						</text>
						<text
							style="width: 45%;display: flex;flex-direction: row;justify-content: center;background-color: #E60012;align-items: center;border-radius: 50rpx;color:#fff;"
						>
							确定
						</text>
					</view>
				...
				<view class="cate-content" @click.stop.prevent="stopPrevent" @touchmove.stop.prevent="stopPrevent">
					...
					<scroll-view scroll-y class="cate-list">
						<view class="cate-flow" v-for="brand in BrandList" :key="brand.CompanyBrandID">
							<view class="cate-item" :class="{ active: brand.CompanyBrandID == SearchParam.CBID }" @click="changeBrand(brand)">{{ brand.CompanyBrandName }}</view>
						</view>
					</scroll-view>
				</view>
			</view>
						...
						<scroll-view scroll-y class="cate-list">
							<view v-for="classy in ClassList" :key="classy.GoodsClassID">
								...
								<view class="cate-flow">
									<view
										v-for="child in classy.children"
										:key="child.GoodsClassID"
										class="cate-item"
										:class="{ active: SearchParam.GCIDS.indexOf(child.GoodsClassID) != -1 }"
...
export default {
	...
	methods: {
		...
		//第一次加载分类
		loadCateList() {
			this.$Request.get(this.$api.goods.getFilterList).then(
				res => {
		...
		loadData(type = 'add', loading) {
			...
			let params = {};
			this.$Request.get(this.$api.goods.getFilterList, params, true).then(
				res => {
